...
# El Veredicto del Contraste 

Un contraste de hipótesis es como un juicio y el **p-valor** es la fuerza de la evidencia presentada..

:::{.callout-tip icon="false" title="Analogía del juicio"}

-   **Hipótesis Nula ($H_0$):** El acusado es inocente (principio de presunción de inocencia).

-   **Evidencia:** Tus datos muestrales.

-   **p-valor:** La probabilidad de que la evidencia en tu contra sea tan fuerte por puro azar (si realmente fueras inocente).

-   **Veredicto:**
    -   **p-valor bajo:** Evidencia muy incriminatoria. Se supera la "duda razonable" ($\alpha$).
        -   **Veredicto $\rightarrow$ Culpable (Rechazamos $H_0$).**
    -   **p-valor alto:** Evidencia débil, circunstancial. No se supera la "duda razonable".
        -   **Veredicto $\rightarrow$ No culpable (No podemos rechazar $H_0$).**

:::

Declarar a alguien "no culpable" no es lo mismo que declararlo "inocente". Simplemente, no teníamos pruebas suficientes para condenarlo. De igual forma, **no rechazar $H_0$ no la convierte en verdadera**, solo significa que no teníamos evidencia suficiente en su contra.
...
|                   | **Verdadera Situación: $H_0$ correcta** | **Verdadera Situación: $H_0$ incorrecta** |
|:------------------|:---------------------------------------|:------------------------------------------|
...
# CH igualdad de proporciones (Planteamiento)

Para comparar dos proporciones, partimos de dos muestras aleatorias de tamaños $n_1$ y $n_2$.

- **Estimadores:** Si $\mathbf{x}$ e $\mathbf{y}$ son los "éxitos" en cada muestra, las proporciones muestrales son:
  $$
  \hat{p}_1=\frac{\mathbf{x}}{n_1} \quad \text{y} \quad \hat{p}_2=\frac{\mathbf{y}}{n_2}
  $$

- **Hipótesis a contrastar** (con un nivel de significación $\alpha$):
  - $H_0: p_1 = p_2$ (Las proporciones poblacionales son iguales).
  - $H_1: p_1 \neq p_2$ (Son distintas).

# CH igualdad de proporciones (Estadístico)

Para decidir sobre $H_0$, calculamos un estadístico de prueba que, bajo $H_0$, sigue una distribución conocida.
...
- **Proporción combinada ($\hat{p}$):** El error estándar se calcula usando una proporción conjunta que asume que $H_0$ es cierta:
  $$
  \hat{p}=\frac{\mathbf{x}+\mathbf{y}}{n_1+n_2}
  $$
